...
models_list <- c(PV_svm, NPV_svm, soil_svm)
rasters_list
band_names <- dimnames(PV_svm$SV)[[2]]

unmix <- function(raster_list, models, bandnames){
  ...
  for (obs in 1:n_bands) {
    ...
    names(pred_pv)   <- obs_dates[obs][1:9]
...
stack_pv <- unmix(rasters_list, models_list, band_names)
